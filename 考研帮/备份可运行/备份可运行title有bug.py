...
def get_m3u8(url):
    ...
    try:
        title = re.findall('cover:"(.*?title:".*?)"',data2)[0]
...
if __name__ == '__main__':
    ...
    i = 1
    j = 0
    second_category = [6,7,8,9]#拥有视频的second_category值
    while True:
        ...
        try:
            print("获取视频网址中。。。。")
            links = get_alllinks(first_url)#多次运行请确保拥有管理员权限删除生成的文件
        except:
            print("获取所有视频网址结束。。。。")
        if len(links) == 0:
            j = j + 1
        ...
        i = i + 1
        for url in links:
            title = get_m3u8(url)   #获取m3u8文件，其中包含ts视频的网址,返回处理过程中的标题
            dir_path = os.path.join(base_path,title)
